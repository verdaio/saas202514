...
```bash
# Database
DATABASE_URL=postgresql://postgres:postgres@localhost:5414/saas202514

# Redis
REDIS_URL=redis://localhost:6414
...
**Navigate to:**
```
https://github.com/YOUR_ORG/saas202514/settings/secrets/actions
```
...
| `AZURE_REGISTRY_NAME` | Container Registry name | `saas202514acr` |
...
az containerapp restart --name saas202514-staging --resource-group saas202514-rg
az containerapp restart --name saas202514-prod --resource-group saas202514-rg
```
...
az ad sp create-for-rbac \
  --name "saas202514-github-actions-new" \
  --role contributor \
  --scopes /subscriptions/{sub-id}/resourceGroups/saas202514-rg \
...
1. Verify secret exists:
   ```bash
   az keyvault secret show --vault-name saas202514-kv --name SECRET-NAME
   ```
...
2. Check access policy:
   ```bash
   az keyvault show --name saas202514-kv --query properties.accessPolicies
   ```
...
az keyvault secret set --vault-name saas202514-kv --name SECRET-NAME --value "value"

# Get secret from Key Vault
az keyvault secret show --vault-name saas202514-kv --name SECRET-NAME --query value -o tsv

# List all secrets
az keyvault secret list --vault-name saas202514-kv

# Delete secret
az keyvault secret delete --vault-name saas202514-kv --name SECRET-NAME
